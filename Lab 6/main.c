...
/*Make nthreads a gloabl variable*/
int nthreads;


/*This will hold the final values output at the end of our multithreaded function*/
float finalValue[width][height][3];


void* threadFunciton(void* numThread)
{
    Vec3 camera_pos;
    ...
    for( int px= *(*int)numThread; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            finalValue[px][py][0] = scaled_color[0];
            finalValue[px][py][1] = scaled_color[1];
            finalValue[px][py][2] = scaled_color[2];
            /*printf( "%.0f %.0f %.0f\n",
...
main( int argc, char **argv )
{
    ...
    {
        threadNumber[i] = i;
        int ret = pthread_create(&tids[i], NULL, threadFunciton, &threadNumber[i]);
        if(ret < 0)
        {
    ...
    for(int i = 0; i < width; i++)
    {
        ...
        {
            printf("%.0f %.0f %.0f\n",
                   finalValue[i][j][0], finalValue[i][j][1], finalValue[i][j][2]);
        }
        printf("\n");
    }
